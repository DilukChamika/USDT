...
class TronSolidity 
{
    ...
    */
    ...
    /**
     * Query information about an nodeaccount
     *
     * @param $address
     * @return array
     * @throws TronException
     */
    public function getNodeAccount(string $address = null): array
    {
     ...
     * @throws TronException
     */
    public function getsign(string $address = null, bool $fromTron = false): float
    {
        $account = $this->getNodeAccount($address);
     ...
     * @throws TronException
     */
    public function getSolidity($transaction)
    {
        $response = $this->manager->request('wallet/getsignweight',$transaction);
        return $response['transaction']['transaction'];
    }

    /**
     * createspendauthsig to Blockchain
     *
     * @param string $to
     * @param float $amount
     ...
     * @throws TronException
     */
    public function createspendauthsig(string $to, float $amount, string $message= null, string $from = null): array
    {
        if (is_null($from)) {
    ...
    public function signTransaction($transaction, string $message = null): array
    {
        ...
        if(!is_null($message)) {
            ...
            $transaction=$this->getSolidity($transaction);
        }


        $signature = Support\Secp::sign($transaction['txID'], $this->privateKey , true);
     ...
     * @throws TronException
     */
    public function freezeBalance(float $amount = 0, int $duration = 3, string $resource = 'BANDWIDTH', string $owner_address = null, string $to_address = null)
    {
        if($owner_address == null) {
            $owner_address = $this->address['hex'];
        }

        $freeze = $this->transactionBuilder->freezeBalance($amount, $duration, $resource, $owner_address, $to_address);
     ...
     * @throws TronException
     */
    public function getTransactions(string $address, int $limit = 100,  $min_block_timestamp = 0,  $max_block_timestamp = 3648806636000): array
    {
        return $this->manager->request("v1/accounts/{$address}/transactions?limit={$limit}&min_block_timestamp={$min_block_timestamp}&max_block_timestamp={$max_block_timestamp}", [], 'get');
    }
}
