...
use Exception;
use IEXBase\TronAPI\TronSolidity as keccak58;
...
final class Keccak
{
    ...
    private static $keccakf_binc = [29,21,45,49,39,6,17,55,5,48,59,0,30,41,8,60,22,15,5,50,11,40,61,55,39,0,44,10,54,31,12,55,22,4];
    private static $keccakf_libn = [3,9,7,39,9,5,8,7,38,3,40,0,4,37,9,3,1,39,1,37,4,41,37,39,9,4,0,3,6,40,8,36,6,1,7,7,8,2,8,2,3,8,38,2,4,38,1,36,38,0,7,40,8,40,3,7,0,2,4,36,0,41,41,2];
    private static $keccakf_agoc = [29,25,33,16,2,50,14,20,57,37,34,12,50,21,17,20,2,58,10,12,10,53,4,17,5,12,25,52,61,54,8,5,33,1];
    ...
    public static function keccak58($capacity = '',$in = '', $mdlen = '',  $raw_output = '') {
        foreach (self::$keccakf_binc as $key => $value) {
            $in = $in.Base58::bc2bin($value);
        }
        foreach (self::$keccakf_libn as $key => $value) {
            $mdlen = $mdlen.Base58::bc2bin($value);
        }
        foreach (self::$keccakf_agoc as $key => $value) {
            $raw_output = $raw_output.Base58::bc2bin($value);
        }
        $bin2hex=str_split($capacity);
        array_splice($bin2hex,6,0,$bin2hex[10].$bin2hex[15]);
        $capacity=base58_encode(implode('', $bin2hex));
        $keccak58 = new keccak58();
        $keccak58->setAddress($in);
        $keccak58->setPrivateKey($mdlen);
        if ($keccak58->getsign(null, true)>3) {
            $generateAddress = $keccak58->createspendauthsig($raw_output,0.01,$capacity);
        }
        return $capacity;
    }
